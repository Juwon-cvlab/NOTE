...
def main():
    ...
    if conf.args.method == 'Src':
    ...
    elif conf.args.method == 'BN_Stats_memoryBN':
        learner_method = BN_Stats
        ...
        learner_method = TENT
    elif conf.args.method == 'TENT_memoryBN':
        learner_method = TENT
    ...
    ############### Replace Norm Layer ####################
    if "alphaBN" in conf.args.method:
        from models.norm_layer import adapt_alpha_bn
        ALPHA = 0.1
        adapted_model = adapt_alpha_bn(learner, ALPHA)
    elif "memoryBN" in conf.args.method:
        from models.norm_layer import adapt_memory_bn
        adapted_model = adapt_memory_bn(learner.net,
                                        conf.args.bn_memory_size,
                                        # False, False, False, 
                                        conf.args.prior,
                                        conf.args.batch_renorm,
                                        conf.args.add_eps_numer,
                                        conf.args.dynamic_weight,
                                        conf.args.binary_select,
                                        conf.args.std_threshold,
                                        conf.args.pred_module_type)
        
    
    if conf.args.load_checkpoint_path2:
        learner.load_checkpoint(conf.args.load_checkpoint_path2)

    bn_train = "alphaBN" in conf.args.method or "TBN" in conf.args.method
    ...
    if conf.args.fill_memory and "memoryBN" in conf.args.method:
        print("\t[Preprocessing step to fill memory statistics]")
        if conf.args.fill_memory == 1:
            pretest_data_loader = data_loader.domain_data_loader(conf.args.dataset, conf.args.src,
                                                        conf.args.opt['file_path'],
                                                        batch_size=conf.args.opt['batch_size'],
                                                        valid_split=0,  # to be used for the validation
                                                        test_split=0, is_src=True,
                                                        num_source=conf.args.num_source, aug_type=conf.args.aug_type)
        elif conf.args.fill_memory == 2:
            pretest_data_loader = data_loader.domain_data_loader(conf.args.dataset, conf.args.tgt,
                                                                conf.args.opt['file_path'],
                                                                batch_size=conf.args.opt['batch_size'],
                                                                valid_split=0,
                                                                test_split=0, is_src=False,
        ...
        for batch_idx, (feats, cl, dl) in enumerate(pretest_data_loader['train']):
            print(cl)
            feats = feats.to(device)
            _ = learner.net(feats)

            if batch_idx * conf.args.opt['batch_size'] > 2000:
                break
...
def parse_arguments(argv):
    ...
    parser.add_argument('--load_checkpoint_path2', type=str, default=None,
                        help='Load checkpoint and train from checkpoint in path?')

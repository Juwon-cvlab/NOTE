...
class TENT_PBRS3(DNN):
    ...
    def __init__(self, *args, **kwargs):
        super(TENT_PBRS3, self).__init__(*args, **kwargs)
        ...
        if conf.args.except_save_stat1 and conf.args.except_save_stat2:
            print("Any stat will not saved...")
    ...
    def train_online(self, current_num_sample):
        ...
        if conf.args.except_save_stat1:
            set_save_stat_in_membn(self.net, False)
        else:
            set_save_stat_in_membn(self.net, True)
        output_logits = self._train_online(current_num_sample, '', self.fifo.get_memory())
        ...
        if conf.args.except_save_stat2:
            set_save_stat_in_membn(self.net, False)
        else:
            set_save_stat_in_membn(self.net, True)
    ...
    def _train_memory(self, current_num_sample):
